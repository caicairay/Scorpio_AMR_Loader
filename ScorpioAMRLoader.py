...
def main():
    ...
    flnm = '/Users/dra/workspace/YT_AMR_Scorpio/3D/g0016.h5'
    periodicity = [False]*3
...
class ScorpioLoader():
    ...
    def _preload(self):
        ...
        missing_dims = 3-self.ndim
        if missing_dims != 0:
             [self.domain.append([0]) for i in range(missing_dims)]
        self.block_data_shape = tuple(([i+2*self.nbuf for i in self.block_dimensions]+[self.nvar])[::-1])
        return
